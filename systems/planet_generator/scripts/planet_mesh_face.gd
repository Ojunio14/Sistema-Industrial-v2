...
func regenerate_mesh(planet_data : PlanetData):
	...
	var color_array := PackedColorArray() # <--- NOVO: Array para as cores
	
	vertex_array.clear()
	uv_array.clear()
	normal_array.clear()
	index_array.clear()
	color_array.clear() # <--- NOVO
	...
	color_array.resize(num_vertices) # <--- NOVO: Redimensiona
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			# 1. Pega a posição (Altura)
			var pointOnPlanet := planet_data.point_on_planet(pointOnUnitSphere)
			vertex_array[i] = pointOnPlanet
			
			# 2. Pega a COR do Bioma (Teste Visual)
			# <--- NOVO: Chamamos a função que criamos no PlanetData
			#color_array[i] = planet_data.get_biome_color(pointOnUnitSphere)
			color_array[i] = planet_data.get_biome_id_for_shader(pointOnUnitSphere)
			#color_array[i] = Color.MAGENTA
			#color_array[i]
	...
	# (Seu código de cálculo de normais continua igual aqui...)
	...
	arrays[Mesh.ARRAY_COLOR] = color_array # <--- NOVO: Passa as cores para a malha
