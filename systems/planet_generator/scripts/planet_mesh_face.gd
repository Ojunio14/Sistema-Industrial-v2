...
func _update_mesh(arrays : Array):
	...
	# [CORREÇÃO AQUI]
	# Duplicamos para garantir independência
	final_material = final_material.duplicate()
	
	# Só tentamos alterar o cull_mode se for um Material Padrão.
	# Se for ShaderMaterial, ignoramos essa linha (faremos isso no shader).
	...
	match CURRENT_MODE:
		...
		ViewMode.HYBRID:
			if final_material:
				# O next_pass já é uma cópia segura pois duplicamos o final_material acima
	...
	# --- COLISÃO ---
	# Só ativa colisão se o Quadtree já dividiu bastante (LOD alto)
